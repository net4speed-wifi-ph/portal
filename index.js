...
jQuery('.brand-name').html(ssidName);
                ...
                function doAuth () {
                    Ajax.post(submitUrl, JSON.stringify(submitData).toString(), function(data){
                        ...
                        if(!!data && data.errorCode === 0) {
                            isCommited = true;
                            if(landingUrl.indexOf('captive') > -1){
                                document.getElementById("oper-hint").innerHTML = errorHintMap[data.errorCode];
                                window.location.href = 'https://piowifi.net/home-portal/?portal_id='+portalID+'&mac='+clientMac+'&code='+document.getElementById("voucherCode").value;
                                window.open('https://piowifi.net/home-portal/?portal_id='+portalID+'&mac='+clientMac+'&code='+document.getElementById("voucherCode").value,'_blank');
                            }else{
                                document.getElementById("oper-hint").innerHTML = errorHintMap[data.errorCode]+" Redirecting...";
                                window.location.href = landingUrl.replace('replacecode', document.getElementById("voucherCode").value);
                            }
                        } else{
                            document.getElementById("oper-hint").innerHTML = errorHintMap[data.errorCode];
                        }
